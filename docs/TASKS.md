...
- 状态：`scaffold_created`（本地 Qt6 Quick 初始工程：`clients/qt_observer`）
- 作用：作为后续 Qt/QML observer cockpit 的客户端工作区，最终应通过 G2a client-agnostic protocol 展示 run status、player panels、event stream、provider trace summary、audit links、God/Role View。
- 当前事实：`clients/qt_observer` 目前只有 Qt Creator 自动生成的 `CMakeLists.txt`、`main.cpp`、`Main.qml` 初始骨架；`Main.qml` 仍是默认窗口，不含 observer protocol integration 或产品 UI。
- 依赖：G2a Local Observer Server / Protocol Control Plane。
- Non-goals：当前 scaffold 不代表 G2b 完成；不直接绑定 Python runtime internals；不做人机 UI，不做 Web client，不做 prompt/profile editor，不做 leaderboard。
