...
| P3-A：Agent Card + Memory Spine | completed | P3-A-1 schema bridge complete; P3-A-2a Flutter local draft role-policy editor, P3-A-2b RolePolicy asset registry, P3-A-2c `prompt_v5` runtime consumption, P3-A-3 AgentContextPacket memory schema, and P3-A-4 `p3a_roleplay_shadow` first playable shadow arm complete |
...
| P3-A-2：Mobile RolePolicy editor | completed | `docs/superpowers/specs/2026-07-05-p3-a-2-mobile-role-policy-editor-design.md`; P3-A-2a complete in Flutter local draft mode: RolePolicyPack-scoped two-column role grid, full-screen detail page, structured presets/settings, local dirty draft state, and read-only engine/runtime composition boundaries. P3-A-2b complete with `src/werewolf_eval/role_policy_registry.py` and `tests/test_role_policy_registry.py`: built-in six-role RolePolicyPack, draft creation, publishing, referenced-policy versioning, JSON round-trip, ownership-field and secret guards. P3-A-2c complete with `src/werewolf_eval/prompt_v5.py`, `tests/test_prompt_roleplay.py`, `tests/golden_prompts/prompt_v5/`, and prompt ledger/golden guards: explicit `prompt_v5` consumes RolePolicy + AgentContextPacket through the renderer path while `PROMPT_VERSION` stays `prompt_v1` |
...
| P3-A-4：First playable roleplay arm(shadow-safe) | completed | `src/werewolf_eval/roleplay_shadow_arm.py`, `src/werewolf_eval/run_emergent_deepseek_game.py`, `src/werewolf_eval/ablation/{arms,harness}.py`, `tests/test_roleplay_shadow_arm.py`; explicit `p3a_roleplay_shadow` arm, role-agnostic SeatCharacterCard prompt block, public/private audit split, wolf-only TeamPlan visibility, baseline-off comparability, and provider-turn call accounting |
