...
- 状态：`next_candidate`（plan 文件 `docs/harness/plans/2026-06-03--g1h-live-runtime-event-spine-plan.md`）
...
- 核心产物：`events.jsonl`、runtime snapshot 文件、prompt manifest、provider lifecycle event records、与 Game Log / Decision Log / Consensus Log / Provider Trace / Failure Audit 的引用兼容关系。
