...
> **村民阵营关键角色（神职）定义** — 本节多处规则按目标是否为「村民阵营关键角色」分档计分。该集合的唯一事实来源是代码 `scoring_types.KEY_VILLAGER_ROLES`，当前为 **{预言家 seer、女巫 witch、猎人 hunter、守卫 guard}**。下方各表枚举均以此集合为准。
>
> **2026-06-12 入账（rules_v1_2 守卫并入关键角色）** — 守卫(guard)随 rules_v1_2 上线归入关键角色集合。计分影响：刀守卫 outcome 由平民档 +1 改为神职档 +3；救守卫 +1→+3；毒守卫 −1→−3；投出守卫按关键角色档计。本条为 2026-06-12 体检 review 确认的**有意编辑**（对齐代码已落的 `KEY_VILLAGER_ROLES` 含 guard），非静默漂移。仅影响含守卫的板，标准 6 人板（无守卫）计分不变。
...
|------|---------|-----------------|----------------|------|
| kill 目标为神职（预言家/女巫/猎人/守卫） | +3 | — | — | 对照终局身份 |
| kill 目标为平民 | +1 | — | — | 对照终局身份 |
...
| 行动 | outcome | decision_quality | rule_integrity | 条件 |
|------|---------|-----------------|----------------|------|
| 投票处决村民关键角色（预言家/女巫/猎人/守卫） | +2 | — | — | 投票结果 + 身份对照 |
| 投票处决平民 | +1 | — | — | — |
| 投票处决狼人同伴 | -2 | — | — | — |
