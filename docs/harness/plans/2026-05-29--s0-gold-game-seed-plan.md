...
**Architecture:** This is a document-only Phase 1 spike task. It creates a stable, reviewable event-chain artifact that S1 can convert into Game Log JSON and S2/S3/S6 can use as fixed input. It does not create business code, runtime code, app/server/web directories, dependencies, or automated game engine logic.
...
- The PR that adds this plan file is plan delivery only.
- The follow-up PR that creates `docs/gold-game/s0-gold-game-seed.md` is the actual S0 Implementation PR.
...
- Create: `docs/gold-game/s0-gold-game-seed.md`
- Modify: none
- Test file: `docs/gold-game/s0-gold-game-seed.md` self-check section added later in Task 7
...
## Task 2: Add fixed game premise and rule assumptions
...
- [ ] **Step 1: Append premise and assumptions**
...
required = [
    ...
    'Day 2 note for scoring conversion:',
    '| Hunter | Not included |',
    '| Witch save potion | One use per game |',
    '| Witch poison potion | One use per game |',
    '| Win condition | Village wins when all werewolves are eliminated |',
...
## Task 3: Add complete event chain
...
- [ ] **Step 1: Append full event chain**
...
assert 'p2\'s public Day 1 support for p1' in text
...
## Task 4: Add S1 conversion notes for evidence references

**Files:**

- Create: none
- Modify: `docs/gold-game/s0-gold-game-seed.md`
- Test file: no separate automated test file; validation command checks visibility and evidence-reference notes

- [ ] **Step 1: Append S1 conversion notes**
...
- [ ] **Step 2: Validate conversion notes**
...
- [ ] **Step 3: Commit checkpoint for S1 conversion notes**

```bash
git add docs/gold-game/s0-gold-game-seed.md
git commit -m "docs: add S0 conversion notes"
```

Expected result:

```text
[branch-name commit-sha] docs: add S0 conversion notes
 1 file changed, ... insertions(+)
```

---

## Task 5: Add completeness assessment
...
## Task 6: Add source and copyright risk assessment
...
## Task 7: Add acceptance check and checkpoint report guidance
...
for item in required:
    assert item in text, item
print('Task 7 validation passed')
PY
```
...
## Task 8: Final repository validation
...
- [ ] **Step 2: Verify only the intended S0 artifact was added by the implementation branch**
...
- `docs/harness/plans/2026-05-29--s0-gold-game-seed-plan.md`
...
for event_type in ['role_assignment', 'werewolf_kill', 'seer_check', 'witch_save', 'witch_poison', 'player_speech', 'player_vote', 'player_eliminated', 'player_died', 'role_revealed', 'game_over']:
    assert event_type in text, event_type
assert text.count('| g001_e') == 38
assert 'Suggested future `visible_info_refs` for g001_e025: `g001_e011`, `g001_e012`, `g001_e017`, `g001_e023`.' in text
assert 'Day 2 village cohesion value is `2 / 2 = 1.0`.' in text
...
- `git diff --check main...HEAD` prints no output.
...
- Naming boundary: This plan path ends with `-plan.md`, so it cannot be confused with the actual S0 artifact.
