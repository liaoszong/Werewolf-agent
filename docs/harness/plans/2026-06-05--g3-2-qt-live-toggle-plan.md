...
- **C1-bis — no stale LIVE.** On `currentRunId` change, on a run-detail response that lacks a string `execution_mode`, and on any detail/capabilities request error, `currentExecutionMode` is **reset to `""`** (→ `SYS: SIMULATION`) and the change signal is emitted — it must **never** retain the previous run's value. (Opening a live run then a fake run must not briefly show `SYS: LIVE_API`.)
...
- [ ] **Step 1 (red):** extend `tests/test_qt_observer_static_contract.py`:
  ...
  - **C1-bis (stale guard):** assert a reset path exists in `.cpp` — one of `m_currentExecutionMode.clear()` / `m_currentExecutionMode = QString()` / `m_currentExecutionMode = QStringLiteral("")`.
  Run → fail.
- [ ] **Step 2 (green):** implement: `launchFromProfile(const QVariantMap&, const QString &mode)` → `body["mode"]=mode`; `refreshCapabilities()` GET `/api/runtime/capabilities` → set properties (on error set `liveAvailable=false`, `liveReasonCode="unreachable"`); in `openRun` parse `execution_mode` → `m_currentExecutionMode` (default `""`). **C1:** never set `m_currentExecutionMode` from intent/202. **C1-bis (stale guard):** clear `m_currentExecutionMode` (`""`) whenever `m_currentRunId` changes and whenever a run-detail response has no string `execution_mode` (and on detail/capabilities error), emitting the change so the chip falls back to `SYS: SIMULATION` — never retain a prior run's value.
...
- **A5** `DataSourceChip` shows executed truth **only** from run-detail `execution_mode` (conservative `SYS: SIMULATION` until it returns, never optimistic-live); `currentExecutionMode` is **reset on run change / missing field / error** so a prior live run can't leave a stale `SYS: LIVE_API`; amber "Deterministic Mock" banner removed. *(T5, C1, C1-bis)*
