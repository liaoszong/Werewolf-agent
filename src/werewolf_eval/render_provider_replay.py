...
def render_provider_replay_html(context: dict[str, Any]) -> str:
    ...
    # -- Player table --
    player_rows = "\n".join(
        _row([p["player_id"], p["role"], p["team"]])
        for p in context["players"]
    )

    # -- Event timeline --
    event_rows = "\n".join(
        _row([e["sequence"], e["round"], e["phase"], e["type"], e["actor"], e["target"], e["summary"]])
    ...
    if context["decision_log_supplied"]:
        d_rows = "\n".join(
            _row([d["decision_id"], d["actor"], d["phase"], d["action"], d["target"], d["reason_summary"]])
            for d in context["decisions"]
        )
        decision_section = f"""<section><h2>Decision Replay</h2>
<div class="scroll"><table>
{_head(["Decision ID", "Actor", "Phase", "Action", "Target", "Reason Summary"])}
{d_rows}
</table></div></section>"""
    ...
    if context["consensus_log_supplied"]:
        c_rows = "\n".join(
            _row([c["consensus_id"], c["round"], c["participants"], c["status"], c["final_target"], c["supporters"], c["dissenters"]])
            for c in context["consensuses"]
        )
        consensus_section = f"""<section><h2>Consensus Replay</h2>
<div class="scroll"><table>
{_head(["Consensus ID", "Round", "Participants", "Status", "Final Target", "Supporters", "Dissenters"])}
{c_rows}
</table></div></section>"""
    ...
    if context["provider_trace"]["has_trace"]:
        ...
        if reqs:
            req_rows = "\n".join(
                _row([r["request_id"], r["actor"], r["phase"], str(r["round"]), r.get("observation_summary", "")])
                for r in reqs
            )
        resp_rows = ""
        if resps:
        ...
        trace_section = f"""<section><h2>Provider Trace</h2>
<p><strong>{_html(t['provider_name'])}</strong> — Requests: {t['request_count']}, Responses: {t['response_count']}, Failures: {t['failure_count']}, Total tokens: {t['total_tokens']}</p>
{f'''<h3>Requests</h3><div class="scroll"><table>
{_head(["Request ID", "Actor", "Phase", "Round", "Observation"])}
{req_rows}
</table></div>''' if req_rows else ''}
{f'''<h3>Responses</h3><div class="scroll"><table>
{_head(["Request ID", "Raw Content", "Latency (ms)", "Tokens"])}
{resp_rows}
    ...
    if context["failure_audit_supplied"]:
        if context["failures"]:
            f_rows = "\n".join(
                _row([f["round"], f["phase"], f["actor"], f["kind"], f["reason"]])
                for f in context["failures"]
            )
            failure_section = f"""<section><h2>Failure Audit</h2>
<div class="scroll"><table>
{_head(["Round", "Phase", "Actor", "Kind", "Reason"])}
{f_rows}
</table></div></section>"""
        else:
            failure_section = """<section><h2>Failure Audit</h2>
<p>zero provider failures</p>
</section>"""
...
<head>
  ...
  <title>Provider Replay — {_html(game['game_id'])}</title>
  <style>
...
<body>
<main>
  <h1>Provider Replay — {_html(game['game_id'])}</h1>
  <p><span class="badge">Source: {_html(source_label)}</span></p>
  <p><em>No live API call is made during rendering.</em></p>

  <section class="warning"><h2>Boundary</h2><p>Replay/report only; not a live observer, not a leaderboard, not a scoring mutation.</p></section>

  <section><h2>Game Summary</h2>
  <p>Game: {_html(game['game_id'])} / Winner: {_html(game['winner'])} / End Round: {_html(game['end_round'])} / Players: {_html(game['player_count'])} / Events: {_html(game['event_count'])} / Source: {_html(source_label)}</p>
  </section>

  <section><h2>Players</h2>
  <div class="scroll"><table>
  {_head(["Player ID", "Role", "Team"])}
  {player_rows}
  </table></div></section>

  <section><h2>Event Timeline</h2>
  <div class="scroll"><table>
  {_head(["Seq", "Round", "Phase", "Type", "Actor", "Target", "Summary"])}
  {event_rows}
  </table></div></section>
