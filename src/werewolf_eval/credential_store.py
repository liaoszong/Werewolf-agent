...
method is `get`, called by the launch wiring (and the capability check).
`__repr__`/`__str__` are redacted so a failing test or debug log can never print a
key. Lock-guarded because the observer runs on a ThreadingHTTPServer."""
