...
**Phase 1 deterministic MVP、Phase 2 evaluator runtime、G1a-G1h provider-backed audit/replay/event-spine foundation 已完成。** 当前 main 已包含 E1 Game Log parser / validator、E2 deterministic scorer、E3 rule attribution engine、E4 runtime demo HTML exporter、D1 Decision Log runtime input、D2 Decision Log deterministic scoring integration、S4 Consensus Log runtime input、S5 saved semantic-label research harness and scoring integration、G1a scripted deterministic fresh-log runner、G1b deterministic game engine + mock agent contract、G1c wolf consensus + failure recovery、G1d fake-provider contract、G1e DeepSeek provider smoke、G1f DeepSeek consensus smoke、G1g provider replay HTML、G1h Live Runtime Event Spine。

G1a-G1h 的当前价值是 audit foundation、replay foundation、runtime event spine foundation、log bundle / provider trace / failure audit foundation。G1g 的 HTML replay/report 只是 offline audit artifact，用于审查和复盘 provider-backed 游戏包；它不是 primary UX，也不代表 observer server、Qt/Web client、prompt/profile editor、人机 UI、multi-provider arena 或 real multi-game Leaderboard 已完成。

G-track 后续路线已在 `docs/ROADMAP.md` 固化：G1h Live Runtime Event Spine 已完成，下一候选开发点是 G2a Local Observer Server / Protocol Control Plane。Qt/QML 是推荐的后续 rich client 方向，但必须通过 client-agnostic protocol 消费 Python runtime/server 输出，不直接绑定 Python 内部对象。

本地已创建 Qt/QML 初始工程骨架 `clients/qt_observer`，用于后续 G2b Qt Observer MVP 起步。该目录目前只是 Qt Creator 自动生成的 Qt6 Quick scaffold，不包含 observer protocol integration、match cockpit、God/Role View、run control 或 replay UI。
...
| [Qt Observer Scaffold](clients/qt_observer/README.md) | `SCAFFOLD` | G2b Qt/QML observer client 起点；当前仅自动生成初始工程 |
